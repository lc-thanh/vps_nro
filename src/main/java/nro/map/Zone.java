...
import nro.main.Menu; // dung de tele sang ban do kho bau: map.id = 135;
...
//                    }
                ...
                if(mapid >= 53 && mapid <= 62 && p.clan != null && p.clan.openDoanhTrai) {
                    ...
                    return;
                } else if(mapid >= 135 && mapid <= 138 && p.clan != null && p.clan.openBDKB) {
                    // NEXT MAP BDKB: 135 -> 138 -> 136 -> 137
                    if(mapid == 135 && p.clan.bdkb[0] != null) {// DI DEN MAP 1
                        Service.gI().leaveOutMap(p);
                        if(map.id == 138) { // map 2 di nguoc ve map 1 [138 -> 135]
                            p.x = (short)1116;
                            p.y = (short)1032;
                        } else { // diem spawn (vi day la map 1)
                            p.x = (short)100;
                            p.y = (short)100;
                        }
                        p.clan.bdkb[0].area[0].Enter(p);
                        return;
                    } else if(mapid == 138 && p.clan.bdkb[1] != null) { // DI DEN MAP 2
//                        if((map.id == 135 && chkAllMobDieDT()) || (map.id == 136)) {
                        if((map.id == 135) || (map.id == 136)) {
                            Service.gI().leaveOutMap(p);
                            if(map.id == 135) { // [135 -> 138]
                                p.x = (short)60;
                                p.y = (short)336;
                            } else { // [136 -> 138]
                                p.x = (short)660;
                        ...
                        return;
                    } else if(mapid == 136 && p.clan.bdkb[2] != null) { // DI DEN MAP 3
                        if((map.id == 138 && chkAllMobDieDT()) || (map.id == 137)) {
                            Service.gI().leaveOutMap(p);
                            if(map.id == 138) { // [138 -> 136]
                                p.x = (short)60;
                                p.y = (short)192;
                            } else { // [137 -> 136]
                                p.x = (short)1380;
                        ...
                        return;
                    } else if(mapid == 137 && p.clan.bdkb[3] != null) { // DI DEN MAP 4 - MAP CUOI
                        if(map.id == 136 && chkAllMobDieDT()) { // [136 -> 137]
                            Service.gI().leaveOutMap(p);
                            p.x = (short)60;
                            p.y = (short)168;
                            p.clan.bdkb[3].area[0].Enter(p);
                        } else {
                            p.sendAddchatYellow("Tiêu diệt hết tất cả trước khi qua ải tiếp theo!");
                            p.map = mapold;
                            Service.gI().resetPoint(p.session, p.x - 50, p.y);
...
//                            int percentDropKichHoat = Util.nextInt(0, 5000);
                                int percentDropItemGod = Util.nextInt(0, 1000); // TI lE ROT DO THAN
//                                int percentDropItemGod = Util.nextInt(0, 5000); // TI lE ROT DO THAN
...
//                                    Player suPhu = PlayerManger.gI().getPlayerByDetuID(_bTarget.id);
                        ...
                        hoiSinhDetu.schedule(hsDetu, 5000);
...
//            }
    ...
    public void chat(Player player, String _text) {
        Message msg;
        try {
            // CAP NHAT STATUS CHO DE TU NEU CHAT "tan cong", "bao ve", "di theo", "ve nha"
            if (!player.isPet && !player.isPet2 && !player.isBOSS && !player.isMabu) { // neu la nguoi choi thi moi thuc hien chuc nang nay
                if (_text.equals("kho bau")) { // qua Dong Hai Tac
                    Map maptele = MainManager.getMapid(135);
                    Controller.getInstance().teleportToMAP(player, maptele);
                }

                byte statuspet = 0; // status de tu 0: di theo, 1 bao ve, 2 tan cong, 3 ve nha, 4 hop the
                if (_text.contains("tan cong")) { // tan cong
                    statuspet = (byte) 2;

                } else if (_text.contains("bao ve")) { // bao ve
                    statuspet = (byte) 1;

                } else if (_text.contains("di theo")) { // di theo
                    statuspet = (byte) 0;

                } else if (_text.contains("ve nha")) { // ve nha
                    statuspet = (byte) 3;
                }
...
//            }
    ...
    public void update() {
        if((map.id < 53 || map.id > 62) && map.id != 147 && map.id != 148 && map.id != 149 && map.id != 151 && map.id != 152 && ((map.id < 135) || (map.id > 138))) {
            synchronized (this) {
                try {
    ...
    public void updateMobAuto() {
        ...
        if(mobs.size() > 0) {
            for(i = 0; i < this.mobs.size(); i++) {
                mob = this.mobs.get(i);
                if(mob != null && !mob.isboss && mob.template.tempId != 70 && mob.template.tempId != 72) {
                    scheduleMobAuto(mob);
                }
            }
        }
...
//                    timerMobAuto.cancel();
                ...
                if(mob.template.tempId == 70 || mob.template.tempId == 71 || mob.template.tempId == 72) {
                    initBigBoss(mob);
                } else {
...
//                        timerHsPro.schedule(ttPro, 60000);
                        timerHsPro.schedule(ttPro, 5000);
...
//                                                }
                                    ...
                                    // AUTO BUFF DAU NEU CO BUA DE TU
                                    if (_player.getBuaDetu()) {
//                                        Controller.UseItemHandler(_player.session, _player, 1, 1, -1 , short idtemp)
                                        if(!_player.isdie){ // neu su phu chua chet
                                            for(int i = 0 ; i < _player.ItemBag.length ;i++){
                                                if(_player.ItemBag[i] != null && _player.ItemBag[i].template.type == 6){
                                                    long _TIMENOW = System.currentTimeMillis();
                                                    if(_player.ItemBag[i].quantity > 0 && ((_TIMENOW - _player._TIMEBUFFDAU) > 10000)){
                                                        _player._TIMEBUFFDAU = _TIMENOW;
                                                        int hpbuff = _player.ItemBag[i].getParamItemByID(48);
                                                        if(hpbuff == 0 ) {
                                                            hpbuff = _player.ItemBag[i].getParamItemByID(2)*1000;
                                                        }
                                                        _player.hp = (_player.hp + hpbuff) > _player.getHpFull() ? _player.getHpFull() : (_player.hp + hpbuff);
                                                        _player.mp = (_player.mp + hpbuff) > _player.getMpFull() ? _player.getMpFull() : (_player.mp + hpbuff);

                                                        if(_player.havePet == 1 && !_player.detu.isdie) {
                                                            _player.detu.hp = (_player.detu.hp + hpbuff) > _player.detu.getHpFull() ? _player.detu.getHpFull() : (_player.detu.hp + hpbuff);
                                                            _player.detu.mp = (_player.detu.mp + hpbuff) > _player.detu.getMpFull() ? _player.detu.getMpFull() : (_player.detu.mp + hpbuff);
                                                            _player.detu.stamina = (short)(_player.detu.stamina + (short)(_player.ItemBag[i].template.level)*100) > (short)1000 ? (short)1000 : (short)(_player.detu.stamina + (short)(_player.ItemBag[i].template.level)*100);
                                                            _player.updateHpDetu(_player, _player.detu);
                                                            if(_player.petfucus == 1 && _player.statusPet != 3 && _player.statusPet != 4) {
                                                                _player.zone.chat(_player.detu, "Con cám ơn sư phụ");
                                                            }
                                                        }
                                                        _player.updateQuantityItemBag(i, 1);
//                                                      _player.ItemBag[i].quantity-=1;
...
//        if(_detu.tiemNang > 0 && (_detu.hpGoc < _detu.getHpMpLimit() || _detu.mpGoc < _detu.getHpMpLimit() || _detu.damGoc < _detu.getDamLimit())) {
        ...
        if (_mob.isDie) {
            ...
            if(_mob.template.tempId != 0) {
                ...
                int percentDrop = Util.nextInt(0, 10);
                int percentDropItemGod = Util.nextInt(0, 5000); // TI LE ROT DO THAN KHI DE TU DANH
                int perTA = Util.nextInt(0, 1000); // TI LE ROT DO AN
    ...
    public void loadBossNoPet(Boss _BOSS) {
        ...
        try {
            ...
            if(_BOSS._typeBoss < (byte)7) {
                Service.gI().sendThongBaoServer("BOSS " + _BOSS.name + " vừa xuất hiện tại " + map.template.name + " khu " + _BOSS.zone.id);
            }
