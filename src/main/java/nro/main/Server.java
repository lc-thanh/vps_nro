...
public class Server {
    ...
    public short cBDKB = 0; // DEM SO MAP BDKB DANG MO
    public short cKhiGas = 0;
    public short maxDoanhTrai = 200;
    public short maxBDKB = 200;
...
//                    short ySo0 = (short)240;
                    ...
                    Service.gI().sendThongBaoServer("BOSS " + _So4.name + " vừa xuất hiện tại " + maps[idMap].template.name + " khu " + IDZONE);

                    maps[idMap].area[IDZONE].loadInfoBoss(_So3);
                    maps[idMap].area[IDZONE].bossMap.add(_So3);
                    Service.gI().sendThongBaoServer("BOSS " + _So3.name + " vừa xuất hiện tại " + maps[idMap].template.name + " khu " + IDZONE);
                    maps[idMap].area[IDZONE].loadInfoBoss(_So2);
                    maps[idMap].area[IDZONE].bossMap.add(_So2);
                    Service.gI().sendThongBaoServer("BOSS " + _So2.name + " vừa xuất hiện tại " + maps[idMap].template.name + " khu " + IDZONE);
                    maps[idMap].area[IDZONE].loadInfoBoss(_So1);
                    maps[idMap].area[IDZONE].bossMap.add(_So1);
                    Service.gI().sendThongBaoServer("BOSS " + _So1.name + " vừa xuất hiện tại " + maps[idMap].template.name + " khu " + IDZONE);
                    maps[idMap].area[IDZONE].loadInfoBoss(_So0);
                    maps[idMap].area[IDZONE].bossMap.add(_So0);
                    Service.gI().sendThongBaoServer("BOSS " + _So0.name + " vừa xuất hiện tại " + maps[idMap].template.name + " khu " + IDZONE);
                    mapTDST = idMap;
                    khuTDST = IDZONE;
