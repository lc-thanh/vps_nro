    ...
    //                    p.menuID = select;
            ...
            case 60:{ //GOKU NUI KHI VANG
                if(p.taskId >= (short)24) {
                    ...
                    p.sendAddchatYellow("Bạn phải hoàn thành xong nhiệm vụ Fide đại ca mới có thể qua được map này");
                }
                break;
            }
            case 56:{ //WHIS TODO
                    ...
                    if (p.menuID == -1) {
                        if (select == 0) {
                            ...
                            if (p.taskId > (short) 23) {
                                ...
                                p.sendAddchatYellow("Phải hoàn thành xong nhiệm vụ Fide đại ca mới có thể vào được cửa hàng");
                            }
                        }
                    }
                }
            ...
            case 13:{
                if(p.map.id == 5) {
                    if(p.menuID != -1)
                    {
                        ...
                        if(p.menuID == 4 && select == 0) {
                            if(p.clan != null) {
                                if(p.clan.openBDKB && p.clan.bdkb[0] != null) {
                                    Service.gI().leaveOutMap(p);
                                    p.x = (short)100;
                                    p.y = (short)100;
                                    p.clan.bdkb[0].area[0].Enter(p);
                                } else {
                                    Service.gI().clientInput(p, "Hãy chọn cấp độ từ 1-110", "Cấp độ hang kho báu", (byte)0);
                                }
                            }
                        }
                    } else { // NEU NHU menuID == -1
                        if(select == 1){
                        ...
                        if(select == 4){
                            if(p.clan != null) {
                                if(p.clan.openBDKB) {
                                    doMenuArray(p,idNpc,"Bang hội con đang ở hang kho báu cấp " + p.clan.lvBDKB + "\ncon có muốn đi cùng họ không?",new String[]{"Đồng ý","Từ chối"});
                                } else {
                                    doMenuArray(p,idNpc,"Đây là bản đồ kho báu hải tặc tí hon\nCác con cứ yên tâm lên đường\nỞ đây có ta lo\nNhớ chọn cấp độ vừa sức mình nhé", new String[]{"Chọn\ncấp độ","Từ chối"});
                                }
                                p.menuID = select;
                            } else {
                                p.sendAddchatYellow("Cậu chưa có bang hội nên không thể tham gia");
...
//            doMenuArray(p,idnpc,"Ta có thể giúp gì cho con",new String[]{"Nói chuyện","Tính năng","Đá nâng cấp","Bang hội"});
            doMenuArray(p,idnpc,"Ta có thể giúp gì cho con",new String[]{"Nói chuyện","Tính năng","BXH","Bang hội","Kho báu\ndưới biển"});
            return;
