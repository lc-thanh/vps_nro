...
//            m.writer().writeInt(5);
    ...
    public void initKuKu() {
        ...
        sendThongBaoServer("BOSS " + _rKuku.name + " vừa xuất hiện tại " + Server.gI().maps[idMap].template.name + " khu " + _rKuku.zone.id);
//        Util.log("INIT KUKU XONG KHU MAP" + Server.gI().maps[idMap].template.name + ", " + IDZONE);
    ...
    public void initMAPDAUDINH() {
        ...
        sendThongBaoServer("BOSS " + _rMapDinh.name + " vừa xuất hiện tại " + Server.gI().maps[idMap].template.name + " khu " + _rMapDinh.zone.id);
//        Util.log("INIT _rMapDinh XONG MAP " + Server.gI().maps[idMap].template.name + ", " + IDZONE);
    ...
    public void initRAMBO() {
        ...
        sendThongBaoServer("BOSS " + _rRambo.name + " vừa xuất hiện tại " + Server.gI().maps[idMap].template.name + " khu " + _rRambo.zone.id);
...
//        Boss _So0 = new Boss(129, (byte)14, xSo0, ySo4);
        ...
        sendThongBaoServer("BOSS " + _So4.name + " vừa xuất hiện tại " + Server.gI().maps[idMap].template.name + " khu " + IDZONE);

        Server.gI().maps[idMap].area[IDZONE].loadInfoBoss(_So3);
        Server.gI().maps[idMap].area[IDZONE].bossMap.add(_So3);
        sendThongBaoServer("BOSS " + _So3.name + " vừa xuất hiện tại " + Server.gI().maps[idMap].template.name + " khu " + IDZONE);
        Server.gI().maps[idMap].area[IDZONE].loadInfoBoss(_So2);
        Server.gI().maps[idMap].area[IDZONE].bossMap.add(_So2);
        sendThongBaoServer("BOSS " + _So2.name + " vừa xuất hiện tại " + Server.gI().maps[idMap].template.name + " khu " + IDZONE);
        Server.gI().maps[idMap].area[IDZONE].loadInfoBoss(_So1);
        Server.gI().maps[idMap].area[IDZONE].bossMap.add(_So1);
        sendThongBaoServer("BOSS " + _So1.name + " vừa xuất hiện tại " + Server.gI().maps[idMap].template.name + " khu " + IDZONE);
        Server.gI().maps[idMap].area[IDZONE].loadInfoBoss(_So0);
        Server.gI().maps[idMap].area[IDZONE].bossMap.add(_So0);
        sendThongBaoServer("BOSS " + _So0.name + " vừa xuất hiện tại " + Server.gI().maps[idMap].template.name + " khu " + IDZONE);
...
//        timerMabu.schedule(Mabu, 10000);
        ...
        // THOI GIAN MO MABU 12h
        Calendar calendar = Calendar.getInstance();
        calendar.set(Calendar.HOUR_OF_DAY, 15);//SERVER 1
        ...
        TimerTask NRSD = new TimerTask() {
            public void run()
            {
                ...
                for(int i = 0; i < Server.gI().maps.length; i++) {
                    if(Server.gI().maps[i].template.id == 126) { // TIM MAP HIRUDEGARN
                        int indexM = -1;
                        for(int ix = 0; ix < Server.gI().maps[i].area[0].mobs.size(); ix++) {
                            if(Server.gI().maps[i].area[0].mobs.get(ix).template.tempId == 70) { // TIM MOB HIRUDEGARN
                                indexM = ix;
                                break;
                            }
                        }
...
//        timerNRSD.schedule(NRSD, 5000);
        ...
        Calendar calendar = Calendar.getInstance();
        calendar.set(Calendar.HOUR_OF_DAY, 23); //SERVER 1
...
//        MapTemplate doanhTraiTemp = map.template.getMapTemplateByIDMap(53);
    ...
    // INIT MAP BDKB
    public void initBDKB(Clan clan, int level) {
//        MapTemplate doanhTraiTemp = map.template.getMapTemplateByIDMap(53);
        Server.gI().cBDKB  = (short)(Server.gI().cBDKB + 1) > (short)Server.gI().maxBDKB ? (short)Server.gI().maxBDKB : (short)(Server.gI().cBDKB + 1);
        clan.cOpenBDKB = (byte)(clan.cOpenBDKB - 1) < (byte)0 ? (byte)0 : (byte)(clan.cOpenBDKB - 1);
        clan.openBDKB = true;
        clan.lvBDKB = level;
        clan.topenBDKB = System.currentTimeMillis();

        // 1. DONG HAI TAC
        MapTemplate mapTemp1 = MapTemplate.getMapTempByIDMap(135);
...
//        map1.area[0].mobs.get(11).isboss = true;
//        map1.area[0].mobs.get(11).hp = map1.area[0].mobs.get(11).maxHp = map1.area[0].mobs.get(11).maxHp*10;
        clan.bdkb[0] = map1;

        // 2. CANG HAI TAC
        mapTemp1 = MapTemplate.getMapTempByIDMap(138);
        map1 = new Map(mapTemp1);
        map1.area[0].updateMobAuto(); // KHOI TAO MOB THUONG

        for(int ix = 0; ix < map1.area[0].mobs.size(); ix++) { // KHOI TAO MOB BOSS: [72]ROBOT BAO VE
            if(map1.area[0].mobs.get(ix).template.tempId == 72) {
                map1.area[0].refreshMob(ix);
                break;
            }
        }
        ...
        if(map1.area[0].mobs.size() > 0) {
            for(Mob _mob: map1.area[0].mobs) {
                if (_mob.template.tempId != 72) {
                    _mob.maxHp = (int)(_mob.maxHp*20*level);
                    _mob.hp = _mob.maxHp;
                } else {
                    _mob.maxHp = (int)(_mob.maxHp*level);
                    _mob.hp = _mob.maxHp;
                }
            }
        }
        ...
        // 3. HANG BACH TUOC
        mapTemp1 = MapTemplate.getMapTempByIDMap(136);
        map1 = new Map(mapTemp1);
        map1.area[0].updateMobAuto();
        ...
        // 4. DONG KHO BAU
        mapTemp1 = MapTemplate.getMapTempByIDMap(137);
        map1 = new Map(mapTemp1);
        map1.area[0].updateMobAuto();
        map1.start();
        //INIT HP, DAME MOB
...
//        Util.log("AJCJKAHCKA :" + level);
        ...
        TimerTask conlaiDT = new TimerTask() {
            ...
            {
                if(clan.openBDKB) {
                    for(byte i = 0; i < clan.bdkb.length; i++) {
                        if(clan.bdkb[i] != null) {
                            byte size = (byte)(clan.bdkb[i].area[0].players.size());
                            for(byte k = 0; k < size; k++) {
                                if(clan.bdkb[i].area[0].players.get(0) != null) {
                                    clan.bdkb[i].area[0].players.get(0).sendAddchatYellow("Còn " + (30 - (int)((System.currentTimeMillis() - clan.topenBDKB)/60000)) + " phút BĐKB sẽ kết thúc");
                                }
                            }
                        }
                    }
...
//        Util.log("AJCJKAHCKA :" + level);
        ...
        TimerTask endDT = new TimerTask() {
            ...
            {
                if(clan.openBDKB) {
                    Server.gI().cBDKB  = (short)(Server.gI().cBDKB - 1) < (short)0 ? (short)0 : (short)(Server.gI().cBDKB - 1);
                    clan.openBDKB = false;
                    //DAY HET NGUOI CHOI RA NGOAI
                    for(byte i = 0; i < clan.bdkb.length; i++) {
                        //ALL PLAYER VE TRAM TAU VU TRU
                        if(clan.bdkb[i] != null) {
                            ...
                            for(byte k = 0; k < size; k++) {
                                if(clan.bdkb[i].area[0].players.get(0) != null) {
                                    clan.bdkb[i].area[0].players.get(0).sendAddchatYellow("Thời gian BĐKB đã kết thúc");
                                    clan.bdkb[i].area[0].goMapTransport(clan.bdkb[i].area[0].players.get(0), (int)clan.bdkb[i].area[0].players.get(0).gender + 24);
                                }
                            }
                        }
                    }
        ...
        timerEndDT.schedule(endDT, 1800000);
        clan.timerBDKB = timerEndDT;
    }

    // LUC NAO LAM XONG BOSS BDKB NHO ADD CHUC NANG NAY!!
    // END BDKB
    public void endBDKB(int idClan) {
        Clan clan = ClanManager.gI().getClanById(idClan);
        if(clan != null) {
            // TIM NGUOI CHOI TRONG ALL MAP KHI GAS DE THONG BAO END KHI GAS
            for(byte i = 0; i < clan.bdkb.length; i++) {
                // THONG BAO KHI GAS SAP KET THUC SAU 30s
                if(clan.bdkb[i] != null) {
                    byte size = (byte)(clan.bdkb[i].area[0].players.size());
                    for(byte k = 0; k < size; k++) {
                        if(clan.bdkb[i].area[0].players.get(0) != null) {
                            clan.bdkb[i].area[0].players.get(0).sendAddchatYellow("BĐKB kết thúc sau 30 giây");
                        }
                    }
                }
            }
...
//        MapTemplate doanhTraiTemp = map.template.getMapTemplateByIDMap(53);
        ...
        MapTemplate mapTemp1 = MapTemplate.getMapTempByIDMap(149); // 1. THANH PHO SATAN
        Map map1 = new Map(mapTemp1);
        map1.area[0].updateMobAuto();
        map1.start();
        //INIT HP, DAME MOB
        ...
        mapTemp1 = MapTemplate.getMapTempByIDMap(147); // 2. SA MAC
        map1 = new Map(mapTemp1);
        map1.area[0].updateMobAuto();
        map1.start();
        //INIT HP, DAME MOB
        ...
        mapTemp1 = MapTemplate.getMapTempByIDMap(152); // 3. VUNG DAT BANG GIA
        map1 = new Map(mapTemp1);
        map1.area[0].updateMobAuto();
        map1.start();
        //INIT HP, DAME MOB
        ...
        mapTemp1 = MapTemplate.getMapTempByIDMap(151); // 4. HANH TINH BONG TOI
        map1 = new Map(mapTemp1);
        map1.area[0].updateMobAuto();
        map1.start();
        //INIT HP, DAME MOB
        ...
        mapTemp1 = MapTemplate.getMapTempByIDMap(148); // 5. LAU DAI LYCHEE
        map1 = new Map(mapTemp1);
        map1.area[0].updateMobAuto();
        map1.start();
        //INIT HP, DAME MOB
